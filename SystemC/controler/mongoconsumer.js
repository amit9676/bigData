...
const consumer = kafka.consumer({ groupId: 'mongo16' })
consumer.connect()
...
consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
        ...
        console.log(mysales[1], bid_town[parseInt(mysales[1]) - 1]['BranchID'], weather,holiday.toString())
        mongoInsert(mysales[1], bid_town[parseInt(mysales[1]) - 1]['BranchID'], weather,holiday.toString())
    }
})
...
function mongoInsert(SaleID, DateInput, weather, holiday, demographic) {
    ...
    MongoClient.connect(mongourl, function (err, db) {
        ...
        dbo.collection("Sales").insertOne(myobj, function (err, result) {
